<script lang="ts">
  ...
  const API_KEY: string = 'sk-X7ZfhjrYtXiRqJZflLYZwcM9EqunebS9SWN5ZDGg';
...
<section class="text-gray-600 body-font">
  <div class="container px-5 py-36 flex flex-wrap items-center justify-center">
    ...
    <div class="lg:w-2/6 md:w-1/2 bg-gray-100 rounded-lg p-8 flex flex-col w-full">
      ...
      <button on:click={() => askBot(theme)} class="text-white bg-blue-500 border-0 py-2 px-8 focus:outline-none hover:bg-blue-600 rounded text-lg">Generate Prompt</button>
      <p class="text-xs text-gray-500 mt-3">*developer not responsible for AI generated results*</p>
    </div>

    <div class="lg:w-3/5 md:w-1/2 border-solid border-2 border-blue-500 p-10 rounded-lg">
      {#if promise}
        {#await promise}
          <p>waiting for bot...</p>
        {:then res}
